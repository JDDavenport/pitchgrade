...
export default function SignUpPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (result.error) { setError(result.error.message || "Sign up failed"); setLoading(false); }
    else { router.push("/pricing"); }
  }

  async function handleGoogle() {
    await signIn.social({ provider: "google", callbackURL: "/pricing" });
  }
    ...
    <div className="min-h-screen flex items-center justify-center px-4 hero-gradient">
      <div className="w-full max-w-sm">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input type="text" value={name} onChange={(e) => setName(e.target.value)}
              className="w-full px-3 py-2 bg-zinc-900 border border-zinc-800 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 text-zinc-100" required />
          </div>
          <div>
          ...
          <div>
            ...
            <input type="password" value={password} onChange={(e) => setPassword(e.target.value)}
              className="w-full px-3 py-2 bg-zinc-900 border border-zinc-800 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 text-zinc-100" required minLength={8} />
          </div>
          <button type="submit" disabled={loading}
            className="w-full py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg font-medium transition-colors disabled:opacity-50">
        ...
        <button onClick={handleGoogle}
          className="w-full py-2.5 border border-zinc-800 rounded-lg font-medium hover:bg-zinc-900 transition-colors flex items-center justify-center gap-2 text-zinc-300">
